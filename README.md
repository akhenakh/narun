...
routes:
  - path: "/hello/"             # Incoming HTTP request path
    ...
    nats_stream: "ORDERS"       # JetStream stream the subject belongs to
    nats_subject: "tasks.ORDERS.hello" # NATS subject to send the request to

  # Add more routes here...
  # - path: "/another/path"
  #   method: "GET"
  #   nats_stream: "PROCESS"
  #   nats_subject: "tasks.PROCESS.fetch"
```
...
2.  Run the consumer, specifying the NATS connection details and the subject/stream/queue it should listen on (these *must* match a route in `config.yaml`):
    ...
    ./hello \
        ...
        -stream "ORDERS" \
        -subject "tasks.ORDERS.hello" \
        -queue "hello-workers" # Queue group for load balancing/durability

    # Or using go run:
    # go run ./consumers/cmd/hello -nats-url "nats://localhost:4222" -stream "ORDERS" -subject "tasks.ORDERS.hello" -queue "hello-workers"
    ```
...
## Example Consumer Handler (`helloHandler`)

This is the core logic within the `consumers/cmd/hello/main.go` example. Notice how it's just a standard `http.Handler`. The `nconsumer.ListenAndServe` function handles all the NATS complexity.
...
func (h *helloHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	// The request 'r' here is reconstructed by nconsumer from the NATS message.
	// The response writer 'w' is a shim provided by nconsumer to capture output.
	...
	if r.URL.Path == "/hello/" && r.Method == "POST" {
		...
		body, err := io.ReadAll(r.Body) // Read from the reconstructed request body
		if err != nil {
		...
		if err := json.Unmarshal(body, &requestPayload); err != nil {
			...
			w.WriteHeader(http.StatusBadRequest) // WriteHeader is captured by the shim
			json.NewEncoder(w).Encode(map[string]string{"error": "Invalid JSON payload"}) // Write is captured
			return
		}
		...
		w.Header().Set("Content-Type", "application/json") // Set headers are captured
		w.WriteHeader(http.StatusOK)                       // Status code captured
		if err := json.NewEncoder(w).Encode(replyPayload); err != nil { // Body captured
			log.Printf("Error encoding JSON response: %v", err)
		}
		return
	}

	// Handle other paths/methods this handler might receive
	log.Printf("Handler received unexpected request: Path=%s, Method=%s", r.URL.Path, r.Method)
	http.NotFound(w, r) // Standard http utility functions work
}

// ... main function using nconsumer.ListenAndServe(opts, &helloHandler{}) ...
```
